...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="DDPM training / sampling on animal dataset")
    ...
    parser.add_argument("--save_path", type=str, default="saved_models/ddpm.pth",
    ...
    # data
    dataset = AnimalDataset(args.data_root, DEFAULT_CLASSES)
    loader = DataLoader(dataset, batch_size=args.batch_size, shuffle=True, drop_last=True)
    print(f"Loaded {len(dataset)} images | device: {device}")

    # model + train
    net = UNet().to(device)
    train(net, loader, epochs=args.epochs, lr=args.lr, loss_type=args.loss_type)
    ...
    # sample
    imgs, _ = sample(net, n=args.n_samples)
    print(f"Generated {args.n_samples} samples (shape {tuple(imgs.shape)})")
